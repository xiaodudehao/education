...
(function ($) {
    ...
    BootstrapPaginator.prototype = {
        ...
        onPageItemJumped : function (event) {
            ...
            if(!isNaN(value) && page > 0 && page != this.currentPage && page <= this.totalPages){//要加 =
                this.$element.trigger("page-clicked", [event, type, page]);//span标签的跳转实现
            }
        },
        ...
        render: function () {
            ...
            var itemHtml = "<span><input id='input_page' type='text' style='width: 30px;height: 20px;' value='"+this.currentPage+"'/>/"+this.totalPages+"</span>";
            // var itemHtml='<select name="" id="sel_page" style="width: 30px;height: 20px;">';
            // for(var ipa=1;ipa<=this.totalPages;ipa++){
            //     itemHtml+='<option value="'+ipa+'">'+ipa+'</option>';
            // }
            // itemHtml+='</select>';
            itemCustom.append(itemHtml);
            listContainer.append(itemCustom);
            var itemlast = $("<li></li>");//创建一个‘跳转’的span
            var itemTz = $("<span>jump</span>").on("click",null,{type: 'jump'},$.proxy(this.onPageItemJumped, this));//绑定点击事件
            //var itemTz = "<span onclick='pageto()'>跳转</span>";
